...
class Atlas2Normal(UVConverter):
    def __init__(self, atlas_size=200, normal_size=512):
        ...
        self.normal_ex = None
        if Path(f'../.mapping_relations/altas2normal_{atlas_size}_{normal_size}.pickle').is_file():
            with open(f'../.mapping_relations/altas2normal_{atlas_size}_{normal_size}.pickle', mode='rb') as f:
    ...
    def convert(self, atlas_tex):
        if self.normal_tex is None or self.normal_ex is None: mapping(atlas_tex)
        if len(self.mapping_relation)==0:
                ...
                for i in range(self.normal_size):
                    ...
                    for j in range(self.normal_size):
                        ...
                        if ex==0:
                            #normal_tex[i, j] = 1
                            #continue
                            if self.barycentric_coordinates_exists(np.array(normal_a), np.array(normal_b), np.array(normal_c), np.array([i/(self.normal_size-1), j/(self.normal_size-1)])):
                                a, b, c = self.barycentric_coordinates(np.array(normal_a), np.array(normal_b), np.array(normal_c), np.array([i/(self.normal_size-1), j/(self.normal_size-1)]))
            ...
            with open(f'../.mapping_relations/altas2normal_{self.atlas_size}_{self.normal_size}.pickle', mode='wb') as f:
