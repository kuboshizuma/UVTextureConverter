...
class Normal2Atlas(UVConverter):
    def __init__(self, normal_size=512, atlas_size=200):
        ...
        self.atlas_tex = None
        self.atlas_ex = None
        if Path(f'../.mapping_relations/normal2atlas_{normal_size}_{atlas_size}.pickle').is_file():
            with open(f'../.mapping_relations/normal2atlas_{normal_size}_{atlas_size}.pickle', mode='rb') as f:
    ...
    def convert(self, normal_tex):
        if self.atlas_tex is None or self.atlas_ex is None: mapping(normal_tex)
            ...
            for k in tqdm(range(len(self.normal_faces))):
                ...
                for i in range(self.atlas_size):
                    ...
                    for j in range(self.atlas_size):
                        ...
                        ex = self.atlas_ex[face_id-1, i, j]
                        if ex==0:
                            if self.barycentric_coordinates_exists(np.array(atlas_a[1:]), np.array(atlas_b[1:]), np.array(atlas_c[1:]), np.array([i/(self.atlas_size-1), j/(self.atlas_size-1)])):
                                ...
                                new_tex = normal_tex[(self.normal_size-1)-int(normal_tex_pos[1]*self.normal_size), int(normal_tex_pos[0]*self.normal_size)]
                                self.atlas_tex[face_id-1, i, (self.atlas_size-1)-j] = new_tex/255

            with open(f'../.mapping_relations/normal2atlas_{self.normal_size}_{self.atlas_size}.pickle', mode='wb') as f:
